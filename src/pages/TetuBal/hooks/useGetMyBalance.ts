...
type UseGetMyBalance = (balancerLpData: any) => {
  balance: any
}

const tetuBal8020Pool = '0x06Df3b2bbB68adc8B0e302443692037ED9f91b42'
const tetuBal8020PoolAbi = [
  {
    inputs: [
...
export const useGetMyBalance: UseGetMyBalance = (balancerLpData) => {
  const { metaMaskStore, web3Store, contractUtilsChainStore } = useStores()
  ...
  useEffect(() => {
    async function getBalance() {
      try {
        ...
        const response = await contract.methods
          // test address with tokens
          // .balanceOf('0xaBa49c1098Cc0FA65bF2393822Ae8A350019f374')
          .balanceOf(metaMaskStore.walletAddress)
          .call()

        const [decimals] = await contractUtilsChainStore.erc20Decimals([tetuBal8020Pool])

        const formatedResponse = +utils.formatUnits(response, decimals)

        // console.log('useGetMyBalance decimaks', decimals)
        // console.log('useGetMyBalance response', response)
        // console.log('useGetMyBalance formatedResponse', formatedResponse)

        //@ts-ignore
        setBalance(formatedResponse * balancerLpData.lpTokenPrice)
      } catch (e) {}
    }
    ...
    getBalance()
  }, [metaMaskStore.walletAddress, balancerLpData])
